    ...
    for dev in CLIENT.get_devices():
        if gi.repository.NM.DeviceWifi == type(dev):
            try:
                ...
                notify("Wifi rescan failed", urgency="critical", icon="emblem-syncthing-error")
...
def rescan_cb(dev, res, data):
    ...
    if dev.request_scan_finish(res) is True:
        notify("Wifi scan complete", icon="emblem-syncthing-active")
    else:
        notify("Wifi scan failed", urgency="critical", icon="emblem-syncthing-error")
    LOOP.quit()
...
def activate_cb(dev, res, data):
    ...
    try:
        ...
        conn = None
    if conn is not None:
        notify("Activated {}".format(conn.get_id()), icon="network-workgroup")
    else:
        notify("Problem activating {}".format(data.get_id()), urgency="critical", icon="dialog-error")
    LOOP.quit()
...
def deactivate_cb(dev, res, data):
    ...
    if dev.deactivate_connection_finish(res) is True:
        notify("Deactivated {}".format(data.get_id()), icon="airplane-mode")
    else:
        notify("Problem deactivating {}".format(data.get_id()), urgency="critical", icon="dialog-error")
    LOOP.quit()
...
def toggle_networking(enable):
    ...
    notify("Networking {}".format("enabled" if enable is True else "disabled"), icon="network-workgroup")
...
def toggle_wifi(enable):
    ...
    notify("Wifi {}".format("enabled" if enable is True else "disabled"), icon="notification-network-wireless")
...
def toggle_wwan(enable):
    ...
    notify("Wwan {}".format("enabled" if enable is True else "disabled"), icon="network-workgroup")
...
def toggle_bluetooth(enable):
    ...
    try:
        ...
        notify("Bluetooth {}".format("enabled" if enable else "disabled"), icon="bluetooth")
...
def delete_cb(dev, res, data):
    ...
    if dev.delete_finish(res) is True:
        notify("Deleted {}".format(dev.get_id()), icon="emblem-remove")
    else:
        notify("Problem deleting {}".format(dev.get_id()), urgency="critical", icon="dialog-error")
    LOOP.quit()
...
def verify_conn(client, result, data):
    ...
    try:
        ...
        if not all([conn.verify(),
                    ...
                    data.verify_secrets()]):
            raise GLib.Error
        notify("Added {}".format(conn.get_id()), icon="emblem-added")
    except GLib.Error:  # pylint: disable=catching-non-exception
        try:
            notify("Connection to {} failed".format(conn.get_id()), urgency="critical", icon="emblem-error")
